...
public:
	virtual void ProcessEvent( Event &event )
	{
		...
		if( event.GetType() == "keydown" && 
			( event.GetPhase() == Rml::Event::PHASE_TARGET || event.GetPhase() == Rml::Event::PHASE_BUBBLE ) )
		{
		...
		else if( event.GetType() == "change" && ( event.GetPhase() == Rml::Event::PHASE_BUBBLE ) ) {
			...
			if( linebreak ) {
				...
				form->Submit( submit->GetAttribute< Rml::String >("name", ""), submit->GetAttribute< Rml::String >("value", "") );
			}
		}
	}
};
