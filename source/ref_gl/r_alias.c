...
static void Mod_AliasBuildMeshesForFrame0( model_t *mod )
{
	...
	for( k = 0; k < aliasmodel->nummeshes; k++ )
	{
		...
		mesh->xyzArray = ( vec4_t * )Mod_Mem_AllocExt( mod->mempool, size, 0, 1);
...
void Mod_LoadAliasMD3Model( model_t *mod, model_t *parent, void *buffer, bspFormatDesc_t *unused )
{
	...
	mod->type = mod_alias;
	mod->extradata = poutmodel = ( maliasmodel_t * )Mod_Mem_AllocExt( mod->mempool, sizeof( maliasmodel_t ), 0, 1 );
	mod->radius = 0;
	mod->registrationSequence = rsh.registrationSequence;
	...
	buf = ( uint8_t * )Mod_Mem_AllocExt( mod->mempool, bufsize, 0, 1);
