...
void R_AddDebugBounds( const vec3_t mins, const vec3_t maxs, const byte_vec4_t color )
{
	...
	if( r_num_debug_bounds > r_debug_bounds_current_size )
	{
		...
		if( r_debug_bounds )
			r_debug_bounds = Q_Realloc( r_debug_bounds, r_debug_bounds_current_size * sizeof( r_debug_bound_t ) );
		else
			r_debug_bounds = Q_Malloc( r_debug_bounds_current_size * sizeof( r_debug_bound_t ) );
	}
