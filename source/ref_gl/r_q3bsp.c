...
static void Mod_LoadLighting( const lump_t *l, const lump_t *faces )
{
	...
	loadmodel_lightmapRects = Mod_Mem_AllocExt( loadmodel->mempool, loadmodel_numlightmaps * sizeof( *loadmodel_lightmapRects ),0,1);
...
static void Mod_PreloadFaces( const lump_t *l )
{
	...
	else
	{
		...
		loadmodel_dsurfaces = in = Mod_Mem_AllocExt( loadmodel->mempool, loadmodel_numsurfaces*sizeof( *in ),0,1);
...
static void Mod_LoadFaces( const lump_t *l )
{
	...
	out = Mod_Mem_AllocExt( loadmodel->mempool, count*sizeof( *out ),0,1 );
...
static void Mod_LoadVertexes( const lump_t *l )
{
	...
	buffer = Mod_Mem_AllocExt( loadmodel->mempool, bufSize,0,1);
...
static void Mod_LoadSubmodels( const lump_t *l )
{
	...
	mod_inline = Mod_Mem_AllocExt( loadmodel->mempool, count*( sizeof( *mod_inline )+sizeof( *bmodel ) ),0,1 );
	...
	case FACETYPE_PATCH:
		{
			...
			buffer = ( uint8_t * )Mod_Mem_AllocExt( loadmodel->mempool, bufSize,0,1);
			bufPos = 0;
...
static void Mod_LoadPatchGroups( const lump_t *l )
{
	...
	patchcount = 0;
	patches = Mod_Mem_AllocExt( loadmodel->mempool, count*sizeof( *patches ), 0 ,1);
	for( i = 0; i < count; i++ ) {
	...
	if( !patchcount ) {
		Mod_Mem_Free( patches );
		return;
	}

	// allocate patch groups to possibly hold all patches individually
	loadmodel_maxpatchgroups = patchcount;
	loadmodel_patchgroups = Mod_Mem_AllocExt( loadmodel->mempool, loadmodel_maxpatchgroups*sizeof( *loadmodel_patchgroups ), 0, 1);
...
static void Mod_LoadLeafs( const lump_t *l, const lump_t *msLump )
{
	...
	for( i = 0; i < count; i++, in++, out++ )
	{
		...
		buffer = ( uint8_t * )Mod_Mem_AllocExt( loadmodel->mempool, size,0,1 );
...
static void Mod_LoadLightArray( void )
{
	...
	out = Mod_Mem_AllocExt( loadmodel->mempool, sizeof( *out )*count,0,1);
...
static void Mod_Finish( const lump_t *faces, const lump_t *light, vec3_t gridSize, vec3_t ambient, vec3_t outline )
{
	...
	for( i = 0, testFog = loadbmodel->fogs; i < loadbmodel->numfogs; testFog++, i++ )
	{
		...
		testFog->visibleplane = Mod_Mem_AllocExt( loadmodel->mempool, sizeof( cplane_t ),16,1 );
