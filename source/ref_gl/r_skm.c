...
#undef H_SWAP
	...
	poutmodel = mod->extradata = Mod_Mem_Alloc( mod->mempool, sizeof( *poutmodel ) );


	// load text
	texts = Mod_Mem_Alloc( mod->mempool, header->num_text + 1 );
	if( header->ofs_text ) {
	...
	pmem = Mod_Mem_Alloc( mod->mempool, memsize );
...
{
	r_skmcachepool = Mod_Mem_AllocPool( r_mempool, "SKM Cache" );
...
static uint8_t *R_AllocSkeletalDataCache( int entNum, int lodNum, size_t size )
{
	...
	if( !best ) {
		best = Mod_Mem_Alloc( r_skmcachepool, sizeof( *best ) );
		best->data = Mod_Mem_Alloc( r_skmcachepool,  size );
		best->size = size;
		best_prev = NULL;
	}
...
void R_ShutdownSkeletalCache( void )
{
	...
	Mod_Mem_FreePool( &r_skmcachepool );
