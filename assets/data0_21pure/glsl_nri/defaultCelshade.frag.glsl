...
void main(void)
{
	...
	QF_ALPHATEST(outColor.a * inColor.a);
		...
		outColor.rgb += vec3(pass.entityColor.rgb) * vec3(qf_texture(u_EntityDecalTexture, v_TexCoord));
	#else
		tempColor = vec4(pass.entityColor.rgb, 1.0) * vec4(qf_texture(u_EntityDecalTexture, v_TexCoord));
	...
	outColor.rgb += vec3(pass.entityColor.rgb) * vec3(qf_texture(u_StripesTexture, v_TexCoord));
#else
	tempColor = vec4(ubo.entityColor.rgb, 1.0) * vec4(qf_texture(u_StripesTexture, v_TexCoord));
	outColor.rgb = mix(outColor.rgb, tempColor.rgb, tempColor.a);
#endif
